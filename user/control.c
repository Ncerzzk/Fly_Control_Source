...
//PID_S ANGLE_SPEED_Z_PID={5,60,1,0,0,5};    //P=10
PID_S ANGLE_SPEED_Z_PID={5,20,0,0,0,5};
...
void Fly_Init(void){      //解锁飞行，初始化高度、yaw
	State=FLY_WAIT;
	yaw_offset=yaw;
	base_duty=20;           //FLY_WAIT状态，占空比可以手动更改
}
...
void Move(float x,float y){
	Limit(x,2.5);
	Limit(y,2.5);
	roll_target=ROLL_CONSTANT+y;
	pitch_target=PITCH_CONSTANT+x;
}
...
void Height_Add(){
	...
	if(height_target<HEIGHT_T&&height>50){
		height_target+=0.1;
	}else if(height_target>HEIGHT_T){
		height_target=HEIGHT_T;
	}
}

int height_ok_flag=0;
void Fly_Control(){
	...
	//这是测试看图算坐标方向是否正确的
//	X_Out=PID_Control(&X_PID,70,position_x);
//	Y_Out=PID_Control(&Y_PID,80,position_y);   //目标位置（原点或者小车应该在画面的中间偏后位置，因为飞机的摄像头偏前）
//	
//	Move(X_Out,Y_Out);
...
//	}
	...
	if(State==FLY||State==LAND){
			...
			Angle_Speed_Z_Out=PID_Control(&ANGLE_SPEED_Z_PID,Yaw_Out,angle_speed_Z);
	}
